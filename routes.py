...
from models import (User, Role, Event, Task, TaskAssignment, ChangeLogEntry, Status,
                    ...
                    PushSubscription) # Ensure PushSubscription is imported
...
# --- Importar a função de envio de notificações push ---
from utils.push_notification_sender import send_push_to_user
# --- Fim Importar ---
from sqlalchemy.exc import IntegrityError # Import IntegrityError for specific database error handling
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Verifica a propriedade 'is_admin' do usuário logado
...
def _can_view_event_helper(user, event):
    ...
    # (event.tasks deve ser carregado com joinedload para evitar N+1 queries)
...
# 4. Há uma permissão direta (user-only) para o usuário neste evento
    # (event.event_permissions deve ser carregado com joinedload)
...
def get_filtered_events(user, search_query, page, per_page, event_filter_type=None, publication_status='all'): # NOVO PARÂMETRO publication_status
    """
    ...
    print(f"--- DEBUG: publication_status solicitado: {publication_status} --- ") # NOVO DEBUG

    base_query = Event.query.options(
        joinedload(Event.status),
        joinedload(Event.category),
        joinedload(Event.author),
        joinedload(Event.event_permissions), # Eager load EventPermissions (user-only)
    ...
    # Sempre filtra por eventos NÃO cancelados, a menos que haja um filtro específico para "cancelados" (que não temos).
    ...
    if user.is_admin:
        ...
        # Administradores:
        # Vêem todos os eventos não cancelados por padrão ('Todos' ou None da home)
        # Podem filtrar por status específicos ('Ativo', 'Realizado', 'Arquivado')
        if event_filter_type == 'Ativo':
            if active_status_obj:
                base_query = base_query.filter(Event.status == active_status_obj)
            ...
            if realizado_status_obj:
                base_query = base_query.filter(Event.status == realizado_status_obj)
            ...
            if arquivado_status_obj:
                base_query = base_query.filter(Event.status == arquivado_status_obj)
        ...
        if event_filter_type == 'Realizado' or event_filter_type == 'Arquivado':
            ...
            return Event.query.filter(false()).paginate(page=page, per_page=per_page, error_out=False) # Retorna conjunto vazio

        # Sempre aplicamos estas condições para não-admins
        base_query = base_query.filter(
            Event.is_published == True,
            # Event.is_cancelled == False já está filtrado no início
            Event.status == active_status_obj # Sempre filtra para 'Ativo' para não-admins
        )
        print("--- DEBUG: Non-admin base query conditions: Event.is_published == True AND Event.is_cancelled == False AND Event.status.name == 'Ativo' ---")

        # Agora, aplicar as condições de visibilidade do usuário (OR)
        # Primeiro, vamos verificar cada condição separadamente para depuração
        
        # Condição 1: Usuário é autor do evento
        is_author_condition = Event.author_id == user.id
        print(f"--- DEBUG:   - Condição 'É Autor': Event.author_id == {user.id}")

        # Condição 2: Usuário está atribuído a alguma tarefa dentro do evento
        ...
        # Condição 3: Usuário tem uma permissão direta de EventPermission
    ...
    # NOVO: Aplicar filtro de publication_status
    ...
    # Search query filtering (applies to both admin and non-admin after initial filters)
    ...
    # Executa a query e loga a contagem de resultados antes de paginar
...
def get_event_publication_counts(user, event_filter_type=None):
    ...
    """
    base_query = Event.query.filter(Event.is_cancelled == False) # Sempre não cancelado

    # Aplica as condições de visibilidade para não-admins
    if not user.is_admin:
        ...
        base_query = base_query.filter(
            ...
            Event.status == active_status_obj
        )
        # Se um não-admin tentar ver "Realizado" ou "Arquivado", as contagens devem ser 0
        ...
        if event_filter_type == 'Ativo':
            ...
            if arquivado_status_obj:
                base_query = base_query.filter(Event.status == arquivado_status_obj)
        # Se event_filter_type for None ou 'Todos', admin já vê todos os não cancelados, então não há filtro de status aqui.
...
def home():
    ...
    # NOVO: Obter o status de publicação do request.args
    publication_status = request.args.get('publication_status', 'all') # Default para 'all'

    # event_filter_type=None fará com que get_filtered_events use a lógica padrão:
    # Admin: todos os não cancelados.
    # Não-Admin: todos os ativos, publicados, não cancelados e vinculados.
    # Passa publication_status para a função
    events = get_filtered_events(current_user, search_query, page, per_page, event_filter_type=None, publication_status=publication_status) # NOVO
    
    # NOVO: Obter as contagens para exibição no filtro
    publication_counts = get_event_publication_counts(current_user, event_filter_type=None) # NOVO

    # Título da página mais genérico, pois o conteúdo varia por perfil
    ...
    return render_template('home.html', events=events, title=title_text, search_query=search_query, 
                           current_filter='home', publication_status=publication_status, # NOVO
                           publication_counts=publication_counts) # NOVO
...
def active_events():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = 5
    publication_status = request.args.get('publication_status', 'all') # NOVO
    
    events = get_filtered_events(current_user, search_query, page, per_page, event_filter_type='Ativo', publication_status=publication_status) # NOVO
    publication_counts = get_event_publication_counts(current_user, event_filter_type='Ativo') # NOVO

    return render_template('home.html', events=events, title='Eventos Ativos', search_query=search_query, 
                           current_filter='active', publication_status=publication_status, # NOVO
                           publication_counts=publication_counts) # NOVO
...
def completed_events():
    ...
    # Passa 'Realizado' para filtrar. get_filtered_events vai negar para não-admins.
    events = get_filtered_events(current_user, search_query, page, per_page, event_filter_type='Realizado', publication_status=publication_status) # NOVO
    publication_counts = get_event_publication_counts(current_user, event_filter_type='Realizado') # NOVO

    return render_template('home.html', events=events, title='Eventos Realizados', search_query=search_query, 
                           current_filter='completed', publication_status=publication_status, # NOVO
                           publication_counts=publication_counts) # NOVO
...
def archived_events():
    ...
    events = get_filtered_events(current_user, search_query, page, per_page, event_filter_type='Arquivado', publication_status=publication_status) # NOVO
    publication_counts = get_event_publication_counts(current_user, event_filter_type='Arquivado') # NOVO

    return render_template('home.html', events=events, title='Eventos Arquivados', search_query=search_query, 
                           current_filter='archived', publication_status=publication_status, # NOVO
                           publication_counts=publication_counts) # NOVO
...
def event(event_id): # <-- ESTA É A ROTA PARA event.html
    # Eager loading para tarefas, seus atribuídos, autor, e as novas permissões (user-only)
    event_obj = Event.query.options(
        ...
        joinedload(Event.tasks).joinedload(Task.attachments).joinedload(Attachment.uploader),
        joinedload(Event.author), # Load event author for permission checks
        joinedload(Event.event_permissions) # Eager load simplified EventPermissions (user-only)
    ).get_or_404(event_id)

    # Revalidação da permissão de visualização do evento, usando a nova função helper
...
# --- INÍCIO DA LÓGICA DE FILTRAGEM DE TAREFAS ATUALIZADA ---
    ...
    # Verifica se o usuário tem uma permissão explícita (EventPermission) para este evento
    # Se EventPermission for carregado com joinedload, podemos iterar sobre ele.
    ...
    if is_admin or is_event_author or has_explicit_event_permission:
        # Admin, autor do evento, ou usuário com permissão explícita no evento veem TODAS as tarefas
        ...
        # Este usuário não é admin, nem autor, nem tem permissão explícita no evento.
        # Ele deve estar vendo o evento APENAS porque está atribuído a uma ou mais tarefas.
        # Portanto, deve ver SOMENTE as tarefas às quais está atribuído.
        for task in event_obj.tasks:
            # Verifica se o usuário logado está atribuído a esta tarefa específica
        ...
        filtered_completed_tasks.sort(key=lambda t: t.completed_at if t.completed_at else datetime.min, reverse=True) # CORRIGIDO AQUI
    # --- FIM DA LÓGICA DE FILTRAGEM DE TAREFAS ATUALIZADA ---
...
# Permissão para upload de anexo (Mantido como estava)
    ...
    return render_template('event.html',
                           ...
                           can_manage_event_permissions=can_manage_event_permissions, # Será true apenas para admin
...
def update_event(event_id):
    ...
    # Verificação de permissão de edição (apenas autor ou admin)
...
def search():
    ...
    if query:
        ...
        # Apply visibility rules directly to the query (simplified)
        if not current_user.is_admin:
            active_status_obj = Status.query.filter_by(name='Ativo', type='event').first()
...
def new_task(event_id):
    ...
    if not available_task_statuses:
        flash("Nenhum status de tarefa disponível. Por favor, crie pelo menos um status do tipo 'Tarefa' na administração (Ex: Ativo, Pendente, etc.)", 'warning')
...
# === CORREÇÃO APLICADA AQUI ===
            ...
            print(f"--- DEBUG: new_task - Transação comitada com sucesso! ---") # Mantido para verificar o commit final
...
def task_detail(task_id):
    ...
    task_obj = Task.query.options(
        ...
        joinedload(Task.event).joinedload(Event.event_permissions), # Eager load permissions for event access check
        joinedload(Task.comments).joinedload(Comment.author) # Carrega comentários e seus autores
    ).get_or_404(task_id)

    event_obj = task_obj.event

    # Verificação de permissão para visualizar a tarefa, usando a função helper para o evento
    if not _can_view_event_helper(current_user, event_obj):
        flash('Você não tem permissão para visualizar este evento.', 'danger')
        abort(403) # Ou return redirect(url_for('main.home')) para uma experiência mais suave
# --- INÍCIO DA LÓGICA DE FILTRAGEM DE TAREFAS ATUALIZADA ---
    ...
    # --- Permissões para controle de botões de EVENTO no template (alguns repetidos, mas por clareza) ---
    can_edit_task = (
        current_user.is_admin or
...
# Preenche o SelectField para task_category com o ID
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            old_assignee_names = sorted([u.username for u in task_obj.assignees]) if task_obj.assignees else [] # Renomeado para uso no histórico
...
# Verifica se os atribuídos mudaram para evitar operações desnecessárias
            ...
            if assignees_changed:
                ...
                if selected_assignee_ids:
                    ...
                    for user_obj in assignee_users:
                        ...
                        print(f"--- DEBUG: update_task - Atribuindo TaskAssignment: Tarefa '{task_obj.title}' para Usuário '{user_obj.username}' ---")
                else:
                    print("--- DEBUG: update_task - Nenhum usuário selecionado para atribuição. ---")
            # === FIM DA CORREÇÃO ===
            print(f"--- DEBUG: update_task - Transação principal de dados processada. Preparando para histórico e commit final. ---")
            ...
            new_assignee_names = sorted([u.username for u in task_obj.assignees]) if task_obj.assignees else [] # Obtem os novos nomes para o histórico
...
def delete_event_permission(permission_id):
    permission = EventPermission.query.get_or_404(permission_id) # Corrigido para buscar EventPermission
    event_id = permission.event_id # Pega o event_id antes de deletar
...
def download_attachment(attachment_id):
    ...
    if current_user.is_admin:
        ...
        can_download = True
    elif current_user.id in [u.id for u in task_obj.assignees]:
        can_download = True
    elif current_user.role_obj and current_user.role_obj.can_view_event: # Tem permissão global de role para ver eventos
        can_download = True
    # Verificação se o usuário tem permissão direta no evento (simplificada)
    elif _can_view_event_helper(current_user, task_obj.event):
        can_download = True
...
def calendar_events_feed():
    ...
    events_query = Event.query.options(
        ...
        joinedload(Event.status),
        joinedload(Event.category),
        joinedload(Event.event_permissions), # Eager load simplified EventPermissions
        joinedload(Event.tasks).joinedload(Task.assignees_associations).joinedload(TaskAssignment.user)
    )
    ...
    if not current_user.is_admin:
        ...
        # Para não-admins, eventos devem estar publicados E ATIVOS
        active_status_obj = Status.query.filter_by(name='Ativo', type='event').first()
        if active_status_obj:
            events_query = events_query.filter(and_(or_(*event_conditions), Event.is_published == True, Event.status == active_status_obj))
        else: # Se não houver status 'Ativo', não mostrar nada para não-admin
    ...
    if not current_user.is_admin:
        ...
        active_status_obj = Status.query.filter_by(name='Ativo', type='event').first()
        if active_status_obj:
            tasks_query = tasks_query.join(Event).filter(and_(or_(*event_conditions), Event.is_published == True, Event.is_cancelled == False, Event.status == active_status_obj))
        else:
            tasks_query = tasks_query.filter(false())
    else: # Admin sees all tasks for non-cancelled events
...
def subscribe():
    ...
    # Acessa p256dh e auth dentro da chave 'keys' do objeto de assinatura
    keys = data.get('keys')
    ...
    try:
        # 1. Tenta encontrar uma assinatura existente para este endpoint (UNIQUE constraint applies here)
        existing_subscription = PushSubscription.query.filter_by(endpoint=endpoint).first()

        if existing_subscription:
            # Se uma assinatura com este endpoint já existe:
            if existing_subscription.user_id != current_user.id:
                # Caso o endpoint esteja associado a outro usuário (cenário raro, mas possível para limpar dados antigos)
                # Reassocia ao usuário atual e atualiza as chaves
                ...
                existing_subscription.timestamp = datetime.utcnow() # Update timestamp on reassociation/update
                db.session.commit()
                return jsonify({'message': 'Assinatura de notificação atualizada e reassociada.'}), 200
            else:
                # Se a assinatura existe e já pertence ao usuário atual, verifica se há mudanças nas chaves
                if existing_subscription.p256dh != p256dh or existing_subscription.auth != auth:
                    existing_subscription.p256dh = p256dh
                    existing_subscription.auth = auth
                    existing_subscription.timestamp = datetime.utcnow() # Update timestamp on update
                    db.session.commit()
                    return jsonify({'message': 'Assinatura de notificação atualizada.'}), 200
                else:
                    # Nenhuma mudança, apenas informa que já está inscrito
                    return jsonify({'message': 'Já inscrito para notificações com este endpoint.'}), 200
        else:
            # Se nenhuma assinatura com este endpoint foi encontrada, cria uma nova
            new_subscription = PushSubscription(
                user_id=current_user.id,
                endpoint=endpoint,
                p256dh=p256dh,
                auth=auth,
                timestamp=datetime.utcnow() # Set timestamp for new subscription
            )
        ...
        db.session.rollback() # Desfaz qualquer operação pendente no banco
        current_app.logger.error(f"IntegrityError ao adicionar/atualizar assinatura: {e}")
        # This catch should ideally not be hit with the prior lookup, but good for defensive coding
